...
def evaluate_sklearn(model, X_test, y_test, models_dir: str = 'models', threshold: float = 0.5):
    y_prob = model.predict_proba(X_test)[:, 1]  # probability of threat class
    y_pred = (y_prob >= threshold).astype(int)

    # Calculate binary cross-entropy loss
    # Clip probabilities to avoid log(0)
    y_prob_clipped = np.clip(y_prob, 1e-15, 1 - 1e-15)
    bce_loss = float(log_loss(y_test, y_prob_clipped))
