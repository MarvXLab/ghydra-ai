...
with st.sidebar:
    ...
    st.markdown("---")
    model_ready = os.path.exists(os.path.join(MODELS_DIR, 'threat_model.keras'))
    if model_ready:
...
def load_model():
    ...
    return tf.keras.models.load_model(os.path.join(MODELS_DIR, 'threat_model.keras'))
...
if page == "📊 Dashboard":
    ...
    results = load_eval()
    if not results:
        st.info("No evaluation results found. Train the model first with `python train.py`.")
    else:
        # Metric cards
        cols = st.columns(5)
        metrics = [
...
# ── Page: Training History ────────────────────────────────────────────────────
    ...
    st.markdown("Loss and accuracy curves across training epochs.")
    st.markdown("---")

    history = load_history()
    if not history:
        st.info("No training history found. Run `python train.py` first.")
        st.stop()
    ...
    # Loss
    axes[0].plot(epochs, history['loss'],     '#58a6ff', label='Train Loss', linewidth=2)
    axes[0].plot(epochs, history['val_loss'], '#f78166', label='Val Loss',   linewidth=2, linestyle='--')
    ...
    # Accuracy
    axes[1].plot(epochs, history['accuracy'],     '#3fb950', label='Train Acc', linewidth=2)
    axes[1].plot(epochs, history['val_accuracy'], '#d29922', label='Val Acc',   linewidth=2, linestyle='--')
    ...
    df = pd.DataFrame({
        ...
        'Accuracy':  [f"{v*100:.2f}%" for v in history['accuracy']],
        'Val Accuracy': [f"{v*100:.2f}%" for v in history['val_accuracy']],
    })
